...
namespace EduSpot.Web.Controllers
{
    public class MajorController : Controller
    {
        ...
        public async Task<IActionResult> MajorIndex()
        {
        ...
        public async Task<IActionResult> UpSet(int? Majorid)
        {
            ...
            {
                ResponseDto? response = await _majorService.GetMajorByIdAsync((int)Majorid);
                if (response != null && response.IsSuccess)
                {
        ...
        public async Task<IActionResult> UpSet(VMUniversityMajor model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (response != null && response.IsSuccess)
                {
                    ...
                    return RedirectToAction(nameof(MajorIndex));
                }
            }
            return View(model);

        }
        public async Task<IActionResult> MajorDelete(int Majorid)
        {
            ResponseDto? response = await _majorService.GetMajorByIdAsync(Majorid);
            if (response != null && response.IsSuccess)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> MajorDelete(MajorDto model)
        {
